...
out_root="${validation_root}/validation_official"
status_root="${validation_root}/official_status"
...
for variant in "$@"; do
  ...
  status_path="${status_root}/${variant}.json"
  test -f "${checkpoint}" || { echo "missing checkpoint: ${checkpoint}" >&2; exit 3; }
  if [[ -e "${out_dir}" || -e "${status_path}" ]]; then
    echo "refusing to overwrite existing official validation output for ${variant}" >&2
    exit 2
  fi
  mkdir -p "${out_dir}"
  write_status "${variant}" running
  command=(
    "${python_bin}" -u "${project_root}/src/semantic_delphi_ukb/evaluate_paper_protocol_medical_auc.py"
