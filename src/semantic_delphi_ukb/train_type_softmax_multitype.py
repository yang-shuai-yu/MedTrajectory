...
out_dir = "ckpt/MedTrajectory_exp2_type_softmax"
eval_interval = 250
log_interval = 25
eval_iters = 25
eval_only = False
...
wandb_run_name = "medtrajectory_exp2_type_softmax_" + str(time.time())
...
dt_loss_weight = 1.0
model_family = "medtrajectory_type_softmax"
...
def main() -> int:
    ...
    token_type_ids, event_type_map = load_token_type_ids(data_dir, vocab_size)
    ...
    if init_from == "scratch":
        model = TypeSoftmaxMultitypeDelphi(TypeSoftmaxMultitypeConfig(**model_args), semantic_embedding_matrix, token_type_ids)
        ...
        model = TypeSoftmaxMultitypeDelphi(TypeSoftmaxMultitypeConfig(**model_args), token_type_ids=token_type_ids)
    ...
    def estimate_loss():
        ...
        for split in ["train", "val"]:
            losses = torch.zeros(eval_iters, 3)
            ...
            for step in range(eval_iters):
                ...
                losses[step] = torch.stack([loss["loss_ce"], loss["loss_type"], loss["loss_dt"]]).detach().cpu()
    ...
    while True:
        ...
        if iter_num % eval_interval == 0 and iter_num > 0:
            ...
            val_loss = (losses["val"][0] + losses["val"][1] + losses["val"][2]).item()
            print(f"step {iter_num}: train loss {losses['train'].sum().item():.4f}, val loss {val_loss:.4f}")
            metrics.update({"val/loss": val_loss, "val/loss_ce": losses["val"][0].item(), "val/loss_type": losses["val"][1].item(), "val/loss_dt": losses["val"][2].item()})
                ...
                torch.save(
                    {
                        ...
                        "config": {**config, "event_type_map": event_type_map},
                    },
        ...
        if iter_num % log_interval == 0:
            print(f"iter {iter_num}: loss {loss['loss'].item():.4f}, ce {loss['loss_ce'].item():.4f}, type {loss['loss_type'].item():.4f}, dt {loss['loss_dt'].item():.4f}, time {(t1 - t0) * 1000:.2f}ms")
            metrics.update({"train/loss": loss["loss"].item(), "lr": lr})
        t0 = t1
